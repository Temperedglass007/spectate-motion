const FEEDS = [
  ...
  { name: 'FIA', url: 'https://www.fia.com/rss-feeds', status: 'confirmed' }
];
...
const attr = (xml, name) => {
  const m = xml.match(new RegExp(`<${name}[^>]*?(?:url|href)=["']([^"']+)["']`, 'i'));
  return m ? m[1] : '';
};
const imageFromHtml = async (url) => {
  try {
    const r = await fetch(url, { headers: { 'user-agent': 'SPECTATE-MOTION/1.0' }, signal: AbortSignal.timeout(5000) });
  ...
  try {
    const settled = await Promise.allSettled(FEEDS.map(async source => {
      ...
      return Promise.all(items.map(async match => {
        const item = match[0];
        const link = tag(item, 'link') || (item.match(/<link[^>]*>([^<]+)<\/link>/i) || [,''])[1].trim();
        return { source: source.name, status: source.status, title: tag(item, 'title'), description: tag(item, 'description').slice(0, 220), link, published: tag(item, 'pubDate') || tag(item, 'published'), image: attr(item, 'media:content') || attr(item, 'enclosure') || await imageFromHtml(link) };
      }));
    }));
    ...
    return res.status(200).json({ updatedAt: new Date().toISOString(), stories: unique });
  } catch (error) {

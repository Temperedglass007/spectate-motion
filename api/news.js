...
export default async function handler(req, res) {
  try {
    const settled = await Promise.allSettled(FEEDS.map(async source => {
      ...
      return Promise.all(items.map(async match => { const item = match[0]; const link = tag(item, 'link') || (item.match(/<link[^>]*>([^<]+)<\/link>/i) || [,''])[1].trim(); return { source: source.name, title: tag(item, 'title'), description: tag(item, 'description').slice(0, 220), link, published: tag(item, 'pubDate') || tag(item, 'published'), image: attr(item, 'media:content') || attr(item, 'enclosure') || await imageFromHtml(link) }; }));
    }));
    ...
    const unique = [...new Map(stories.map(s => [s.link, s])).values()].slice(0, 18).map(s => ({ ...s, status: 'confirmed' }));
    res.setHeader('Cache-Control', 's-maxage=300, stale-while-revalidate=900');
    res.setHeader('Access-Control-Allow-Origin', '*');
    return res.status(200).json({ updatedAt: new Date().toISOString(), stories: unique, policy: 'Only sourced reports are shown in the automated Newsroom feed. SPECTATE analysis and speculation must be explicitly labelled.' });
